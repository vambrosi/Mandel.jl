...
# ╔═╡ 6eb0f4e9-b71c-4472-b9c2-3623af894785
using GLMakie, Symbolics, StaticArraysCore, LinearAlgebra

# ╔═╡ 7116063a-cfb4-11ee-2b9b-8d4f83bddf2b
md"""
# SphereViewer

Plots Julia and Mandel sets on the sphere.
"""

# ╔═╡ 8eb29e7e-52e4-4b78-bc13-d207e51be0be
md"""
## Tests
"""
...
md"""
## Viewer3D definitions
"""

# ╔═╡ 128eb380-b46d-4873-ab2a-5502f39c2cfc
"""
	Viewer3DOptions(ε, max_iter)
"""
...
# ╔═╡ 172f02fd-02d8-4462-81db-f9a39cef317c
md"""
...
# ╔═╡ 9edaa9a9-259e-408b-844a-f99ae38a4247
"""
	distance(pt1, pt2)

Distance between two normalized points in the complex projective line.
"""
function distance(pt1::Point, pt2::Point)
	return norm(pt1[1] * pt2[2] - pt1[2] * pt2[1])
end 

# ╔═╡ 99d20f18-e3cd-464c-8cd6-5a7b7c252144
function projectivize(z::Num, c::Num, expr::Num)
	@variables u, v
...
# ╔═╡ c8aa4683-ca76-44f9-a383-67870d4afcd9
md"""
## Dynamics of a Rational Functions
"""

# ╔═╡ 3d47e084-ddfe-4116-9fc2-0aad4b3f091d
"""
	RationalMap(z, c, expr)
"""
struct RationalMap
	f::Function
	...
	function RationalMap(z, c, expr)
		f = projectivize(z, c, expr)

		df_dz = expand_derivatives(Differential(z)(expr))
		df_variable = projectivize(z, c, df_dz)

		df_dc = expand_derivatives(Differential(c)(expr))
		df_parameter = projectivize(z, c, df_dc)

		return new(f, df_variable, df_parameter, z, c, expr)
	end
end

# ╔═╡ 0d665414-a627-4a44-ada7-12238099be5a
function repeated_iterate(
	f::Function,
	pt::Point,
	c::Number,
	ε::Real,
...
# ╔═╡ 63b543e4-8a32-4adc-965c-a252d29d8513
function near_again(
	f::Function,
	pt1::Point,
	pt2::Point,
...
# ╔═╡ 1643dcee-14d6-44a1-9aa3-2ac0c8feea9c
function iterate_until_close(
	f::Function,
	pt1::Point,
	pt2::Point,
	c::Number,
...
# ╔═╡ 922b8c1d-2341-4661-a0a8-caef6ff0594d
const DEFAULT_VALUE = 0.5

# ╔═╡ 8d0f3ff2-c010-4301-8b80-b97c0d3ff42b
function multiplier(
	f::Function,
	pt::Point,
	c::Number,
    ...
    time_diff, slow, fast = repeated_iterate(f, pt, c, ε, max_iter)
	time_diff > max_iter && return DEFAULT_VALUE 

	period, slow, fast = near_again(f, slow, fast, c, ε, time_diff)
	period > time_diff && return DEFAULT_VALUE

	preperiod = iterate_until_close(f, pt, fast, c, ε, max_iter)
	preperiod > max_iter && return DEFAULT_VALUE 
...
# ╔═╡ ddfe2f5a-ed99-473e-a8a2-957b607de34d
function update!(
	texture::Observable{Matrix{Float64}},
	grid::Matrix{Point},
	f::Function, 
...
struct Viewer3D
	...
	function Viewer3D(z, c, expr, parameter)
		...
		initialize!(texture, grid, rational_map.f,
			parameter, options)
		...
		focus = lift(cam.eyeposition) do eye
			return normalize(convert(SVector{3, Float64}, eye))
		end
		...
		reset = Button(figure[2,1][1,3], label="Reset")
	
		on(zoom_in.clicks) do event
			M = zoom_mobius(focus[], 0.5)
			update!(texture, grid, rational_map.f,
				parameter, options, M)
		end
	
		on(zoom_out.clicks) do event
			M = zoom_mobius(focus[], 2.0)
			update!(texture, grid, rational_map.f,
				parameter, options, M)
		end

		on(reset.clicks) do event
			initialize!(texture, grid, rational_map.f,
				parameter, options)
		end
...
manifest_format = "2.0"
project_hash = "73e21235c6988a22b2c2c60a1e9e1c9966fcd7da"
...
git-tree-sha1 = "31571f931b22f0ebb98cace13b74c0d4516c8c2b"
