...
function pick_orbit!(
    ...
    julia.points[] = orbit(d_system.f, z, julia.parameter, options.orbit_length - 1)
    return julia
end
...
function update_view!(view::View, d_system::DynamicalSystem, options::Options)
    ...
    update_grid!(view, d_system.f, d_system.df_dz, d_system.crit, corner, step, options)
...
struct Viewer
    ...
    function Viewer(
        ...
        mandel =
            MandelView(center = mandel_center, diameter = mandel_diameter, pixels = 1000)
